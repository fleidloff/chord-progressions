			...
			<div className="pure-g Navigation">
			    <div className="pure-u-1 pure-u-sm-2-5">
			  		<select onChange={actions.onChangeSelectedNote} id="note" value={state.selected.note}>
			        	{[ "random", ...notes ].map((note, idx) => <option key={idx} value={note}>{note}{note === "random" && state.selected.note === "random" ? ` (${state.note})` : ""}</option>)}
			        </select>
			    </div>
			    ...
			    <div className="pure-u-1 pure-u-sm-2-5">
			        <select onChange={actions.onChangeSelectedScale} id="scale" value={state.selected.scale}>
			        	{[ "random", ...scales ].map((scale, idx) => <option key={idx} value={scale}>{scale}{scale === "random" && state.selected.scale === "random" ? ` (${state.scale})` : ""}</option>)}
			        </select>
		        </div>
			</div>
			<br />
